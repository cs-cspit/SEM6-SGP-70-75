...
import { getClasses, getQuiz, updateQuiz } from '@/lib/api';
...
export default function EditQuizPage() {
    ...
    const loadData = async () => {
        try {
            const [classesData, quizData] = await Promise.all([
                getClasses(),
                getQuiz(params.id as string)
            ]);

            if (classesData) {
                setClasses(classesData.filter((c: Class) => c.teacher_id === user!.id));
            }
            ...
            if (quizData) {
                setFormData({
                    ...
                    class_number: quizData.class_number || '',
                    total_marks: quizData.total_marks,
                    time_limit: quizData.time_limit ? quizData.time_limit.toString() : '',
                });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateQuiz(params.id as string, {
                ...formData,
                teacher_id: user!.id,
